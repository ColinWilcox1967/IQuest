...
void draw_edging ( int x1, int y1, int x2, int y2)
{
    ...
    {
       WriteChar (x1,y1,TL_SINGLE); WriteChar (x2,y1,TR_SINGLE);
       WriteChar (x1,y2,BL_SINGLE); WriteChar (x2,y2,BR_SINGLE);
       for (i=y1+1; i <= y2-1; i++)
       {
          WriteChar (x1,i,V_SINGLE); WriteChar (x2, i, V_SINGLE);
       }
       for (i=x1+1; i<= x2-1;i++)
       {   
          WriteChar (i, y1, H_SINGLE);
          WriteChar (i, y2, H_SINGLE);
       }
    }
    else
    {
       WriteChar (x1,y1,TL_DOUBLE); WriteChar (x2,y1,TR_DOUBLE);
       WriteChar (x1,y2,BL_DOUBLE); WriteChar (x2,y2,BR_DOUBLE);
       for (i = y1+1; i<=y2-1; i++)
       {
           WriteChar (x1,i, V_DOUBLE); WriteChar (x2,i,V_DOUBLE);
       }
       for (i = x1+1; i <= x2-1; i++) 
       {                     
           WriteChar (i, y1, H_DOUBLE);
           WriteChar (i, y2, H_DOUBLE);
       }
    }
}
...
{
   if ( (config_byte1 & MSK_WINDOW_TYPE ) == MSK_WINDOW_TYPE) 
        explode_window (x1, y1, x2, y2);                         
    draw_box (x1, y1, x2, y2);
    if ( (config_byte1 & MSK_SHADOW_TYPE) == MSK_SHADOW_TYPE)    
...
void draw_box (int x1, int y1, int x2, int y2 )
{
    ...
    if ( (config_byte1 & MSK_BORDER_TYPE) == MSK_DOUBLE_LINE_BORDER)
    {
     ...
     {
         if ( (config_byte1 & MSK_BORDER_TYPE) == MSK_SINGLE_LINE_BORDER)
         {
...
void explode_window (int x1, int y1, int x2, int y2)
{
    ...
    while ( new_y-delta > y1) /* Was >= */
    {
        /* Maintain screen aspect ratio when exploding */

        if ( x2-x1+1>y2-y1+1)
                draw_box ( (int)(new_x-aspect_ratio * delta), (int)(new_y-delta), (int)(new_x+aspect_ratio * delta), (int)(new_y+delta)); 
        else
                draw_box ( (int)(new_x - delta), (int)(new_y - aspect_ratio * delta), (int)(new_x + delta), (int)(new_y + aspect_ratio * delta) );
        delta++;
    }
}
