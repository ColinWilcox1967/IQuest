	     ...
	     switch (ch)
	     {
		   ...
		   case K_CSR_UP    : if (CurrentRow >= FirstLine)
		    ...
		    case K_HOME      : SetInitView ();
				       ...
				       clear_area(5, 4, 79, 20);
				       break;
		    case K_F1       : InfoBuffer = (char *)malloc (PORTSIZE (9, 4, 70, 20));
				      if (InfoBuffer != NULL)
				      {
					     save_area (9, 4, 70, 20, InfoBuffer);
					     SetColourPair (palette.black, palette.green);
					     draw_window (9, 4, 69, 19);
					     window_title (9, 69, 4, "List Of Active Keys", palette.red, palette.green);
					     SetColourPair (palette.black, palette.green);
					     ...
					     centre_push_button (9, 69, 18, "&Continue");
					     restore_area (9, 4, 70, 20, InfoBuffer);
					     free ((char *)InfoBuffer);
				       }
				       break;
		  case K_ALT_I       : InfoBuffer = (char *)malloc (PORTSIZE (5, 3, 76, 12));
				       if (InfoBuffer != NULL)
				       {
				       ...
				       break;
		  case K_ALT_S       : TextSearch (FileName, config_byte1 & MSK_FILE_VIEWER);
				       break;
		  default          :
				       break;
	     }
...
void ShowView ()
{
	...
	while ((i < RightMargin) && (ch != 10))
	{
	     if ( ch == 9) /* TAB */
	     {
		write_blanks ((int)(6 + i - LeftMargin), (int)(4 + l - FirstLine), 4);
		i+=4;
	     }
	     else
	     {
...
void DrawViewerOutline ()
{
   ...
   if ( config_byte1 & MSK_BORDER_TYPE ) /* Single Line */
   {
      WriteChar (1, 3, (char)195);
      WriteChar (80, 3, (char)180);
      WriteMultiChar (2, 3, (char)H_SINGLE, 78);     
	
      WriteChar (4, 3, (char)194);
      for (i = 4; i <= 20; i++)
	  WriteChar (4, i, (char)V_SINGLE);

      WriteChar (1, 21, (char)195);
      WriteChar (80, 21, (char)180);
      WriteMultiChar (3, 21, (char)H_SINGLE, 2); 
      WriteMultiChar (2, 21, (char)H_SINGLE, 78);

      WriteChar (52, 1, (char)194);       
      WriteChar (52, 2, (char)V_SINGLE);
      WriteChar (52, 3, (char)193);

      WriteChar (4, 21, (char)197);
      WriteChar (4, 22, (char)V_SINGLE);
      WriteChar (4, 23, (char)193);
   }
   else
   {
      WriteChar (1, 3, (char)204);
      WriteChar (80, 3, (char)185);
      WriteMultiChar (2, 3, (char)H_DOUBLE, 78);     
	
      WriteChar (4, 3, (char)203);
      for (i = 4; i <= 20; i++)
	  WriteChar (4, i, (char)V_DOUBLE);

      WriteChar (1, 21, (char)204);
      WriteChar (80, 21, (char)185);
      WriteMultiChar (3, 21, (char)H_DOUBLE, 2);
      WriteMultiChar (2, 21, (char)H_DOUBLE, 78);
...
{
    SetColourPair (palette.black, (short)colour);
